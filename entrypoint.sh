#!/bin/sh
# Universal Router v5 — multi-process entrypoint.
#
# Internal port layout:
#   :9009  universal-router (public)
#   :9019  cybergym_agentx          (CyberGym)
#   :8080  agentx-osworld           (OSWorld)
#   :9020  pi_bench_agentx          (Pi-Bench)
#   :9021  malt_agentx              (MALT / NetArena)
#   :8090  fwa_agentx               (FieldWorkArena — new in v5)
#
...
(
    ...
    export QWEN_API_BASE_URL="https://api.deepinfra.com/v1/openai"
    export QWEN_MODEL="Qwen/Qwen3.5-397B-A17B"
    export JEDI_API_BASE_URL="https://costumes-batman-reliance-assured.trycloudflare.com/v1"
    export JEDI_MODEL="jedi-7b"    
...
# ─── 5) FWA / FieldWorkArena (new in v5) ──────────────────────────────────────
echo "[entrypoint] starting fwa_agentx (v1) on :${FWA_PORT}..."
(
    # fwa-agentx supports --host/--port flags AND PORT env var.
    # Uses OpenAI direct API (GPT-5.4 pinned to 2026-03-05 snapshot).
    # FWA_MODEL_PRIORITY default: "gpt-5.4-2026-03-05,gpt-5.4,gpt-5.5"
    export OPENAI_API_KEY="${OPENAI_API_KEY}"
    export FWA_MODEL_PRIORITY="${FWA_MODEL_PRIORITY:-gpt-5.4-2026-03-05,gpt-5.4,gpt-5.5}"
    exec /app/fwa-agentx --host 127.0.0.1 --port "${FWA_PORT}"
) &
FWA_PID=$!
# fwa-agentx exposes both /health and /.well-known/agent-card.json — health is faster.
wait_health "fwa_agentx" "${FWA_PORT}" "${FWA_PID}" 30 || exit 1

# ─── 6) Universal Router (foreground) ─────────────────────────────────────────
echo "[entrypoint] all 5 specialists healthy — starting universal-router on :${ROUTER_PORT}..."
...
export UPSTREAM_VISION_QA="http://127.0.0.1:${FWA_PORT}"
